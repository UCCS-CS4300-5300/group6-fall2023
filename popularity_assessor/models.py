...
class InstagramAccount(models.Model):
    ...
    token = encrypt(models.CharField(
        max_length=255, default=None))  # token for this account on instagram)
    previous_likes = models.IntegerField(
        default=0)  # previous likes on the last post
    previous_followers = models.IntegerField(
        default=0)  # previous followers on the last post
    previous_date = models.DateTimeField(
        auto_now_add=True)  # previous date of the last post
